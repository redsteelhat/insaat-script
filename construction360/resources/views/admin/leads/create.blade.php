...
<div class="space-y-6">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-900">Yeni Lead Ekle</h1>
    </div>

    <form method="POST" action="{{ route('admin.leads.store') }}" class="bg-white rounded-lg shadow p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="status" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
                    <option value="new" {{ old('status') == 'new' ? 'selected' : '' }}>Yeni</option>
                    <option value="contacted" {{ old('status') == 'contacted' ? 'selected' : '' }}>İletişime Geçildi</option>
                    <option value="site_visit_planned" {{ old('status') == 'site_visit_planned' ? 'selected' : '' }}>Keşif Planlandı</option>
                    <option value="quoted" {{ old('status') == 'quoted' ? 'selected' : '' }}>Teklif Verildi</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="source" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
                    ...
                    <option value="telefon" {{ old('source') == 'telefon' ? 'selected' : '' }}>Telefon</option>
                    <option value="referral" {{ old('source') == 'referral' ? 'selected' : '' }}>Referans</option>
                    <option value="sosyal_medya" {{ old('source') == 'sosyal_medya' ? 'selected' : '' }}>Sosyal Medya</option>
                    <option value="diger" {{ old('source') == 'diger' ? 'selected' : '' }}>Diğer</option>
                </select>
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-2">Mesaj / İhtiyaç Detayı</label>
                <textarea name="message" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">{{ old('message') }}</textarea>
            </div>
        ...
        <div class="mt-6 flex justify-end gap-3">
            <a href="{{ route('admin.leads.index') }}" class="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                İptal
            </a>

...
@section('title', 'Tedarikçi Düzenle: ' . $vendor->name)

@section('content')
<div class="space-y-6">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('admin.vendors.show', $vendor) }}" class="text-gray-500 hover:text-gray-700">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
        </a>
        <h1 class="text-3xl font-bold text-gray-900">Tedarikçi Düzenle</h1>
    </div>

    <form method="POST" action="{{ route('admin.vendors.update', $vendor) }}" class="bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $vendor->category) }}" placeholder="beton, demir, seramik, elektrik vb."
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            </div>
            ...
            <div>
                ...
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $vendor->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-teal-600 focus:ring-teal-500">
            ...
            <div>
                ...
                <input type="text" name="tax_office" value="{{ old('tax_office', $vendor->tax_office) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            </div>
            ...
            <div>
                ...
                <input type="url" name="website" value="{{ old('website', $vendor->website) }}" placeholder="https://..."
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="address" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">{{ old('address', $vendor->address) }}</textarea>
            </div>
            ...
            <div class="md:col-span-2 mt-4">
                ...
                <textarea name="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">{{ old('notes', $vendor->notes) }}</textarea>
            </div>
        </div>

        <div class="mt-6 flex justify-end gap-3">
            <a href="{{ route('admin.vendors.show', $vendor) }}" class="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                İptal
            </a>
